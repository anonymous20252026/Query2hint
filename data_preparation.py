...
data_preparation.py
===================
Generates Reptile meta-learning triplets from raw benchmark execution logs.

For each workload (JOB, CEB) reads the per-query execution log CSV and
constructs (anchor, positive, negative) triplets based on which hint
configuration ran faster for each query.  The triplets are used to
meta-train AdaptSteer-R (Reptile encoder).

Inputs:
    benchmark/dataset/job.csv   — JOB execution log
    benchmark/dataset/ceb.csv   — CEB execution log

Outputs:
    stage1_triplets_JOB.csv
    stage1_triplets_CEB.csv
