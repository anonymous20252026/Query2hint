...
====================
PyTorch neural-network heads for binary hint steering.

Defines lightweight MLP classifiers used as the binary decision head
on top of frozen encoder embeddings.  Two architectures are provided:

  BCE_2H_v1_Model  — 2-hidden-layer MLP  (input → 25 → 1)
  BCE_3H_v1_Model  — 3-hidden-layer MLP  (input → 100 → 50 → 1)
  BCE_2H_BN_Model  — same as v1 with BatchNorm regularisation

All models output a single logit; apply sigmoid for probability.
In the main paper the SVC-120-S sklearn pipeline is used instead;
these modules support the neural-head ablation in exp1_supervision_ablation.py.
"""
