...
========================
PyTorch neural-network heads for multi-action hint steering.

Defines MLP classifiers for K-class configuration selection,
used as the multi-action decision head on top of frozen encoder
embeddings.  Two architectures are provided:

  ER_2H_v1_Model — 2-hidden-layer MLP  (input → 25 → 49, softmax)
  ER_3H_v1_Model — 3-hidden-layer MLP  (input → 100 → 50 → 49)

The 49-dim output corresponds to the 49 optimizer hint configurations
evaluated in the multi-action steering experiments.
In the main paper the Condorcet pairwise ranker (exp3b) outperforms
these direct multiclass heads, especially at large K.
"""
import torch
