...
encoder_training.py
===================
Fine-tunes sentence-transformer encoders for AdaptSteer.

Trains two encoder variants from `all-mpnet-base-v2`:

  AdaptSteer-C: contrastive TripletLoss training on Stage-1 triplets.
                Saved to finetuned_models/mpnet_contrastive/

  AdaptSteer-R: same contrastive training followed by Reptile
                meta-learning across JOB and CEB episodes.
                Saved to finetuned_models/mpnet_reptile/

Inputs:
    stage1_triplets_JOB.csv  (from data_preparation.py)
    stage1_triplets_CEB.csv  (from data_preparation.py)
