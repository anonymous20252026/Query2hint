...
=====================
Generates Stage-1 contrastive (anchor / positive / negative) triplets
for training AdaptSteer-C with TripletLoss.

For each unique SQL query the script ranks hint configurations by mean
execution time and constructs triplets:
    anchor   = SQL query text
    positive = fastest hint configuration label
    negative = slowest hint configuration label

Inputs:
    job.csv   — execution log for the JOB workload

Outputs:
    stage1_contrastive_dataset.csv
