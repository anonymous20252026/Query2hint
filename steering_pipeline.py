...
====================
Main AdaptSteer inference pipeline.

Encodes an incoming SQL query with the fine-tuned sentence-transformer
encoder, retrieves nearest-neighbour training queries, runs the SVC-120-S
binary classifier, and returns the recommended hint configuration
(PostgreSQL default or the learned alternative).

The pipeline is designed for online use: typical latency < 13 ms per
query on a single GPU (see experiments/exp6_inference_efficiency.py).

Usage:
    python steering_pipeline.py --benchmark benchmark/queries/job
"""
import os
import re
import sys
